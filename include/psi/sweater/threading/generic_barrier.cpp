...
void generic_barrier::arrive() noexcept
{
    ...
    if ( PSI_UNLIKELY( everyone_arrived ) )
        event_.notify_one();
}
...
#endif // PSI_SWEATER_USE_CALLER_THREAD
    std::scoped_lock<mutex> lock{ mutex_ };
    while ( PSI_UNLIKELY( counter_.load( std::memory_order_relaxed ) != 0 ) )
        event_.wait( mutex_ );
}
...
#endif
{
    ...
    while ( spin_tries-- )
    {
        if ( PSI_LIKELY( everyone_arrived() ) )
            return ( nop_spin_count - spin_tries ) < ( nop_spin_count / 8 );
        nops( 8 );
    }

    while ( PSI_UNLIKELY( !everyone_arrived() ) )
    {
