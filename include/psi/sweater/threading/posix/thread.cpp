...
void thread_impl::join  ()       noexcept { BOOST_VERIFY( pthread_join  ( handle_, nullptr ) == 0 ); handle_ = {}; }
void thread_impl::detach()       noexcept { BOOST_VERIFY( pthread_detach( handle_          ) == 0 ); handle_ = {}; }
thread_impl::native_handle_type thread_impl::get_id() const noexcept { return handle_; }
...
int thread_impl::create( thread_procedure const start_routine, void * const arg ) noexcept
{
    auto const error{ pthread_create( &handle_, nullptr, start_routine, arg ) };
    if ( PSI_UNLIKELY( error ) )
    {
